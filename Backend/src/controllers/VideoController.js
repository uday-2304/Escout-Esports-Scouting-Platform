...
const GAME_PLAYLISTS = {
  ...
  FreeFire: ["UUhLeCIE6M49IgW0anxMm4lw", "UUrPezsltlsZZiEkxTE2l39g"],
...
export const getPlayersDatabase = async (req, res) => {
  try {
    ...
    const videos = await Video.find()
      .populate("uploadedBy", "name") // User model uses 'name', not 'username'
      .sort({ createdAt: -1 });
    ...
    videos.forEach((v) => {
      ...
      grouped[gameKey].push({
        ...
        snippet: {
          ...
          channelTitle: v.uploadedBy?.name || "Arena Player",
